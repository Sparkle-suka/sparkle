...
class GismeteoWeatherMod(loader.Module):
    """Погода для Heroku-UB (Полная исправленная версия)"""
    strings = {
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(
            "default_city", "Прохладный", "Город по умолчанию для .weather"
        )
    ...
    async def render_weather(self, target, lat, lon, city_name, days=0):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                ...
                btns = [
                    [
                        {"text": "📅 3 Дня", "callback": self._cb_handler, "args": (lat, lon, city_name, 3)},
                        {"text": "📅 7 Дней", "callback": self._cb_handler, "args": (lat, lon, city_name, 7)}
                    ],
                    [{"text": "🗑 Скрыть и удалить", "callback": self._cb_handler, "args": (lat, lon, city_name, -1)}]
                ]

                # Универсальная отправка для твоей сборки
                try:
    ...
    def _get_ico(self, code):
        """Правильные иконки: никакой зимы летом!"""
        if code == 0: return "☀️"
