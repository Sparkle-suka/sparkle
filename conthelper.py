...
class ConthelperMod(loader.Module):
    ...
    strings = {
        "name": "Conthelper",
        "blocked": "<b>{} добавлен в чёрный список.</b>",
        "unblocked": "<b>{} удалён из чёрного списка</b>",
        "delcontact": "<b>{} удалён из контактов</b>",
        "who_to_block": "<b>Indicate, кого блокировать?</b>",
        "who_to_unblock": "<b>Indicate, кого разблокировать.</b>",
        "who_to_delcontact": "<b>Indicate, кого удалить из контактов.</b>",
    }
    ...
    async def reportcmd(self, message):
        """Пользователь получил репорт за спам."""
        ...
        await message.edit("<b>Вы получили репорт за спам!</b>")

    async def blockcmd(self, message):
        """Используй: .block для блокировки пользователя."""
        args = utils.get_args_raw(message)
        reply = await message.get_reply_message()
        if message.chat_id != (await message.client.get_me()).id and message.is_private:
            user = await message.client.get_entity(message.chat_id)
    ...
    async def unblockcmd(self, message):
        """Используй: .unblock для разблокировки пользователя."""
        args = utils.get_args_raw(message)
        reply = await message.get_reply_message()
        if message.chat_id != (await message.client.get_me()).id and message.is_private:
            user = await message.client.get_entity(message.chat_id)
    ...
    async def delcontcmd(self, message):
        """Используй: .delcont для удаления из контактов."""
    ...
    async def addcontcmd(self, message):
        """Используй: .addcont для добавления в контакты."""
        args = utils.get_args_raw(message)
        reply = await message.get_reply_message()
        if not args:
        ...
        try:
            ...
            return await message.edit(
                "<b>Something went wrong (come up with different reasons).Что-то пошло не так.</b>"
            )